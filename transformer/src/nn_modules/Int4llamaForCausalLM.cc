...
Int4LlamaForCausalLM::Int4LlamaForCausalLM(std::string param_path, const struct model_config config) {
    allocate_aligned_memory(logits_output, config.max_sqlen * config.vocsize * sizeof(float)); //这个非常小,2048*32000*4
    allocate_aligned_memory(lm_head_weight, (config.embed_dim * config.vocsize * sizeof(uint8_t)) / 2); //这个实际上也小,4096*32000*1
    fprintf(stderr,"  %d, %d  ",config.max_sqlen * config.vocsize * sizeof(float),(config.embed_dim * config.vocsize * sizeof(uint8_t)) / 2);
